...
from util import get_h5_files, H5Dataset, evaluate
...
class LogisticRegressionModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear = nn.Linear(input_size, 1)  # Single output neuron for binary classification
    
    def forward(self, x):
        return self.linear(x)  # No softmax; BCEWithLogitsLoss expects raw logits

print('Defining model, criterion, optim, and scheduler...')
model = LogisticRegressionModel(1024).to(device)
criterion = nn.BCEWithLogitsLoss()
optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-4)  # L2 regularization
